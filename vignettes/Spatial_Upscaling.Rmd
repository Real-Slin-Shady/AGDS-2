...
This project implements the excercise Spatial Upscaling from the AGDS 2
...
Therefore folds are just checking if the prediction task is applicable to
the training-dataset at hand over the entire space spanned by the
...
geographic space in this case. They also perform a feature cross alidation
...
Directly from AGDS 1 forest definition, no hyperparameter tuning is
...
First, we evaluate the distribution of the datasource.
...
the Americas. Probably due to little data availability in these regions.
...
We observe similar distributions of the mean with cluster 2 being very
dense around the mean leaf nitrogen. But the density for some clusters is
much more dense then for others. This might imply that some clusters are
more similar in regards to leaf nitrogen than others.
...
environmental cross validation folds. But now, the density is more even for
the clusters. This is probably because leaf nitrogen is now split more
...
The best metrics of all model has the random cross validation model. This
...
though not representative for the spatial upscaling task. The RSQ and the
RMSE is higher with the environmental cross validation. This probably means
that the folds are more similar to each other and thus have a higher
...
vastly different to each other while in the enviromental upscaling the
folds are more similar. Additionally by far the lowest predictive power of
any cluster has a fold in the spatial upscaling further backing this up.

All in all the preferred solution should be a spatial or enviromental
...
be to check if the new enviromental covariates that are present in each
...
areas with known enviromental covariates. This is because models perform
poorly when exposed to data outside of the training values. (Ludwig et
al.2023) This is also done with the AOA in the paper.
